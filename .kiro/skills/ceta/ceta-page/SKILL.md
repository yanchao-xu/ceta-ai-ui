...
列表页的固定结构：`{ form: { title, labelLayout }, fields: [Card > Table] }`

## 列表页 schemaJson 模板（重要）

列表页的核心是 Table 组件。必须包含在 Card 组件内，且需要 ACTION_COLUMN 定义操作按钮。

**注意：所有页面的 `form` 对象都必须显式设置 `"labelLayout": "vertical"`，不可省略。**
...
**PBC 内部跳转（默认，`hrefIsSiteBased` 省略或为 `false`）：** href 不带 PBC token 前缀，平台自动拼接当前 PBC 的 basename。

| 场景 | href 格式 |
| ---- | --------- |
| 跳转到 PBC 内的 Page | `/page/{schemaId}` |
| 新建表单 | `/form/{formToken}/layout/{layoutToken}` |
| 编辑表单 | `/form/{formToken}/layout/{layoutToken}/:id` |
| 查看表单 | `/form/:id/view` |

**跨 PBC 跳转（`hrefIsSiteBased: true`）：** href 必须带目标 PBC token 前缀。

| 场景 | href 格式 |
| ---- | --------- |
| 跳转到其他 PBC 的 Page | `/{pbcToken}/page/{schemaId}` |
| 跳转到其他 PBC 新建表单 | `/{pbcToken}/form/{formToken}/layout/{layoutToken}` |
| 跳转到其他 PBC 编辑表单 | `/{pbcToken}/form/{formToken}/layout/{layoutToken}/:id` |
| 跳转到其他 PBC 查看表单 | `/{pbcToken}/form/:id/view` |

**关键规则：同一 PBC 内的页面/表单间跳转，href 不带 `/{pbcToken}` 前缀。跨 PBC 跳转才加前缀，并且必须设置 `hrefIsSiteBased: true`。**
