...
| 属性 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| colNumber | number | 列数（表单字段布局最多不超过 4；页面级精确布局可用 12 列栅格） | 2 |
...
- 表单中字段并排显示（最常见 `colNumber: 2`），`colNumber` 最多不超过 4
- 通常嵌套在 Card 或 Collapse item 内部
- 页面级精确布局用 `colNumber: 12`（类似 Bootstrap 12 列栅格），这是唯一允许超过 4 的场景
